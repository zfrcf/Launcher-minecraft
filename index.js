...
async function connect (options) {
  ...
  try {
    ...
    if (!cfg.azureClientId) {
      loadingScreen.status = 'Configure azureClientId in config.json before signing in with Microsoft.'
      loadingScreen.style = 'display: block;'
      loadingScreen.hasError = true
      return
    }
    msaStatus = (data) => {
      const code = data && (data.userCode || data.code)
      const url = data && (data.verificationUri || data.verification_uri)
      if (code) {
        loadingScreen.status = `Microsoft: open ${url || 'https://www.microsoft.com/link'} and enter code ${code}`
      }
    }
  } catch (e) {
  ...
  const bot = mineflayer.createBot({
    ...
    username,
    auth: 'microsoft',
    authTitle: (await fetch('/config.json').then(r => r.json())).azureClientId,
    deviceType: 'Win32',
    onMsaCode: msaStatus,
    viewDistance: 'tiny',
